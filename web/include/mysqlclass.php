...
class SQL {
	...
	function SQL($connString, $user = "", $pass = "") {
		...
		if ($this->adapter == "sqlite" && class_exists("PDO") && in_array("sqlite", PDO::getAvailableDrivers())) {
			$this->method = "pdo";
		...
		} else if ($this->adapter == "sqlite" && substr(sqlite_libversion(), 0, 1) == "2" && class_exists("PDO") ) {
			$this->method = "pdo";
